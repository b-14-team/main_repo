...
$(document).ready(function () {
    ...
    window.showModal = function (columnId) {
        document.getElementById('modal').style.display = 'flex';
        loadAssignees('taskAssignee');
...
function addColumnToBoard(columnData) {
    ...
    newColumn.addEventListener('dragover', (e) => {
        e.preventDefault();
        draggedCard = e.dataTransfer.dropEffect = 'move';
    });
        ...
        success: function (response) {
            ...
            if (Array.isArray(response)) {
                response.forEach(column => {
                    ...
                    columnMap[column.columnsStatus] = {
                        id: column.id,
                        color: column.color,
...
function addCard(cardDto, columnId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            alert("생성 완료");
            console.log("Card creation response:", response); // 서버 응답 로깅
            addCardToColumn(response.data); // 응답 받은 카드로 칼럼에 추가
        },
        error: function (error) {
...
function addCardToColumn(card) {
    var columnId = columnMap[card.columnsStatus];
    console.info("card.columnStatus: ", card.columnStatus);
    ...
    if (columnId) {
        ...
        if (column) {
            ...
            console.error("Column with ID is not found.");
        }
    } else {
...
function deleteCard(cardId) {
    $.ajax({
        ...
        success: function (response) {
            alert("카드가 삭제되었습니다.");
            removeCardFromDOM(cardId); // DOM에서 카드 제거
            //location.reload();
        },
        error: function (error) {
...
function removeCardFromDOM(cardId) {
    var cardElement = document.querySelector(`.card[data-id='${cardId}']`);
    if (cardElement) {
...
function updateCard(cardId, cardDto) {
    $.ajax({
        ...
        success: function (response) {
            alert("카드가 수정되었습니다.");
            updateCardUI(cardId, response.status); // UI 업데이트 함수 호출
...
function createColumn() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            addColumnToBoard(response); // DOM에 새로운 칼럼 추가
...
function updateCardUI(cardId, status) {
    var cardElement = document.querySelector(`.card[data-id='${cardId}']`);
    var targetColumn = document.getElementById(`column${status}`);
